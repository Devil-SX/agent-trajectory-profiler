...
def _format_standard(stats: SessionStatistics, session_id: str) -> str:
    ...
    if stats.tool_groups:
        ...
        if multi_tool_groups:
            ...
            for g in multi_tool_groups:
                ...
                lines.append(
                    "    "
                    f"{g.group_name:<28} {g.count:>5}  {lat_str:>8}  "
                    f"{err_str:>6}  {g.tool_count:>5}"
                )
    ...
    if stats.bash_breakdown:
        ...
        for cs in bb.command_stats[:top_n]:
            ...
            lines.append(
                "    "
                f"{cs.command_name:<20} {cs.count:>5}  {tot_lat:>10}  "
                f"{avg_lat:>8}  {out_str:>8}"
            )
    ...
    if stats.time_breakdown:
        ...
        lines.append(
            "    "
            f"Interactions: {tbd.user_interaction_count}  "
            f"({tbd.interactions_per_hour:.1f}/hour)"
        )
