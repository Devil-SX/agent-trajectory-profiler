...
- Cross-ecosystem parser normalization regression tests (`tests/test_parser_normalization.py`) covering timestamp fallback parity, tool-result truncation semantics, and core text-message behavior alignment between Codex and Claude adapters.
...
- Removed dead table-era leftovers from frontend source (`SessionCard.tsx/.css`, `SessionSelector.tsx/.css`, and `useDebounce.ts`) and aligned SessionBrowser styling copy with the active table-first interaction model.
...
- Replaced stale `frontend/tests/session-card-interaction.spec.ts` card-selector assertions with table-mode session-selection flow coverage (URL-selected default, overview-to-detail switching, metadata/statistics visibility), restoring `@full` targeted stability for this spec.
