...
test.describe('@smoke Statistics Dashboard - Tool Errors', () => {
  ...
  test('should render error timeline with expandable details', async ({ page }) => {
    ...
    await expect(page.locator('.card-title', { hasText: 'Leverage Ratio' })).toBeVisible();
  ...
  test('@smoke formats token magnitudes in cards and token tooltips', async ({ page }) => {
    ...
    await expect(tokenDistributionCard).toContainText('Input Tokens');
    await expect(tokenDistributionCard).toContainText('Output Tokens');
    await expect(tokenDistributionCard.locator('.recharts-sector').first()).toBeVisible();
  });
});
